...
import { PlaylistRefreshProvider } from "./contexts/PlaylistRefreshContext";
...
function App() {
  ...
  const triggerRefresh = useCallback(() => setPlaylistRefreshKey((k) => k + 1), []);

  const handleCreatePlaylist = async (name: string, description: string) => {
    await appAPI.createPlaylist({ name, description });
  ...
  return (
    <PlaylistRefreshProvider value={{ refreshKey: playlistRefreshKey, triggerRefresh }}>
